"""
This is the file that produces a Lenia glider within a trial world
"""
...
def trilobite(size, r):
    ...
    trilobite_world = np.zeros([size, size])
    scale = 1;  cx, cy = 20, 20
    C = scipy.ndimage.zoom(C, scale, order=0);  r *= scale
    trilobite_world[cx:cx+C.shape[0], cy:cy+C.shape[1]] = C

    return trilobite_world
